...
function generateDeviceListHtml(deviceList){
	...
	if (deviceList.length > 0) {			
		for ( var i = 0; i < deviceList.length; i++) {
			...
			devListHtml += "<td><a href='/device/deviceinfo.htm' data-public-params='"+device_id+"' class='initAjax Table-Data-Name-Link'>"+ device_id +"</a>";
			...
			devListHtml += "<td><span class='Table-Data-Name' id='sysload_" + device_id + "'>"+ sysLoad +"</span></td>";
			devListHtml += "<td><span class='Table-Data-Name' id='sysmemfree_" + device_id + "'>"+ sysMemfree +"</span></td>";
			devListHtml += "<td><span class='Table-Data-Flow Table-Flow-Up'><span></span><div id='up_" + device_id + "'>"+ trafficFormatter(upTraffic) +"</div></span>";
			devListHtml += "<span class='Table-Data-Flow Table-Flow-Down'><span></span><div id='down_" + device_id + "'>"+ trafficFormatter(downTraffic) +"</div></span></td>";
			devListHtml += "<td class='ChinaNet-Form-Sheet Width-For-Button' id='dev_opt_btn_" + device_id + "' style='display:none'>";
			//devListHtml += "<a href='javascript:;' data-deviceid='"+device_id+"' data-status='"+status+"'><span><span class='Setting-Name'>设置</span><span>&nbsp;</span></span></a>";
			devListHtml += generateDeviceOptBtn(device_id, status);
			devListHtml += "</td>";
			devListHtml += "</tr>";
		}			
	}
