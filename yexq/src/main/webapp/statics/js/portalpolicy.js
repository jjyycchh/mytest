...
function onShowPolicyData(){
    $('div.ChinaNet-Settings-Button a').each(function(x){
        ...
        var status = $(this).attr('data-status');        
            ...
            $('a#'+id).xiMenu({
            menuItem:[
                {url:'/merchant/editportalpolicy.htm?policyId='+id,className:'initAjax',text:'编辑'},
...
function generatePortalListHtml(ppList) {
    ...
    if (typeof ppList!='undefined'&&ppList!=null&&ppList.length>0) {
        for ( var i = 0; i < ppList.length; i++) {
            ...
            ppListHtml += "    <td><a href='/merchant/editportalpolicy.htm?policyId="+policyid+"' data-public-params='"+policyid+"' class='initAjax Table-Data-Name-Link'>"
