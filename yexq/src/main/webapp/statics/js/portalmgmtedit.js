...
var _TemplateIndustryData = null;
var portalmgmteditApp = function(){
    		...
    		success : function(data) {
    			if (data.result != 'FAIL') {
    				if (isNotEmptyString(data.template)) {
    					...
    					var initTpIndustry = data.template.tags;
    					//console.log(initTpIndustry[0]);
    					if(typeof initTpIndustry == 'undefined'){   						
    						$('#input_templateindustry').xiSelect({offsetSize:[0,3,0,3],data:_TemplateIndustryData.dataList,defaultData:{value:'others',text:'其他行业模板'}});
    					}else if(!isNotEmptyString(initTpIndustry)){
    						$('#input_templateindustry').xiSelect({offsetSize:[0,3,0,3],data:_TemplateIndustryData.dataList,defaultData:{value:'others',text:'其他行业模板'}});
    					}else{
    						$('#input_templateindustry').xiSelect({offsetSize:[0,3,0,3],data:_TemplateIndustryData.dataList,defaultData:{value:initTpIndustry[0],text:_TemplateIndustryData.data[initTpIndustry[0]]}});
    					}
    					if(_initAuthTypeID){
    ...
    var SaveTemplateData = function(){
    	$('#btn_save_pt').die().live('click',function(e){
        	...
        	var tpIndustry = $("#input_templateindustry").val();
        	//console.log(tpname);
        	...
        	if(!isNotEmptyString(tpIndustry)){
        		onAlertErrorTip('请选择行业分类', document.getElementById('div_templateindustry'));
                return false;
        	}
    ...
    var initPortalSelect = function(){
		...
		_TemplateIndustryData = {dataList:[{value:'restaurant',text:'餐饮行业模板'},{value:'finance',text:'银行行业模板'},{value:'travel',text:'旅游行业模板'},{value:'others',text:'其他行业模板'}], data:{restaurant:'餐饮行业模板',finance:'银行行业模板',travel:'旅游行业模板',others:'其他行业模板'}};
		//console.log('2222222');
    }
    ...
    return {init:function(){    	
    	...
    	if(__DATA_PUBLIC_KEY != null){
    		...
    		$('#input_templateindustry').xiSelect({offsetSize:[0,3,0,3],data:_TemplateIndustryData.dataList});
    	}
    	SaveTemplateData();   	
    }}
}();