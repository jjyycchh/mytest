...
var telecomvirtualdeviceApp = function() {
	 ...
	 * 初始化“添加”按钮
	 * */
	var initAddTelecomApBtn = function(){
...
function spliceHtml(){
	...
	addTelecomDeviceHtml += "		<input type='text' id='input_password' class='Input-Control'>";
	addTelecomDeviceHtml += "	  </div>"
	addTelecomDeviceHtml += "  </div>";
	...
	addTelecomDeviceHtml += "</div>";
	return addTelecomDeviceHtml;
}
function addTelecomAp() {
	
	var addTelecomDeviceHtml=spliceHtml();	
	var d_addRadiusDevice = dialog({
		id: 'Dailogin:addTelecomAp',
		title: '添加telecom设备',
...
function submitTelecomDevice() {
	...
	if(!onCheckEmpty(username)){
		onAlertErrorTip('账号名不能为空', document.getElementById('input_username'));
		return false;
	...
	if(!onCheckEmpty(recommend_number)){
	...
	if(ethport.length>20){
		onAlertErrorTip('数据过长', document.getElementById('input_ethport'));
		return false;
	}
	$.ajax({
		type: 'POST',
		...
		success: function (data) {
			if (data.result == 'OK') {
				...
				deviceSearchHandler.clearResultSetpageNo();
				deviceSearchHandler.searchWithPreload();
				dialog.list['Dailogin:addTelecomAp'].remove().close();
			} else {				   
...
function deviceListHtml(deviceList) {
	...
	if (deviceList.length > 0) {
		for (var i = 0; i < deviceList.length;i++) {
			...
			var telecomAccount = deviceList[i].telecomAccount;
...
function loadComButton(i) {
	var jsonObj2 = {ComButton:[]};
	 ...
	 jsonObj2.ComButton.push({url:'javascript:showTelecomDetails(\'' + i + '\');', text:'编辑'});
	 return ComButton;
}

/********************************操作-编辑**********************/
function showTelecomDetails(i){
	...
	deviceDetailsHtml += "    <div class='Form-Item-Input ChinaNet-Col-9 Form-Item-Disabled'>";
	...
	deviceDetailsHtml += "	<label class='Form-Item-Title ChinaNet-Col-2'>设备名称</label>";
	deviceDetailsHtml += "	  <div class='Form-Item-Input ChinaNet-Col-9'>";
	deviceDetailsHtml += "		<input type='text' id='input_name' class='Input-Control'>";
	deviceDetailsHtml += "	  </div>"
	deviceDetailsHtml += "  </div>";
	
	deviceDetailsHtml += "  <div class='ChinaNet-Form-Sheet ChinaNet-Form-Required'>";
	deviceDetailsHtml += "	<label class='Form-Item-Title ChinaNet-Col-2'>宽带账号</label>";
	deviceDetailsHtml += "	  <div class='Form-Item-Input ChinaNet-Col-9'>";
	deviceDetailsHtml += "		<input type='text' id='input_telcomaccount' class='Input-Control'>";
	deviceDetailsHtml += "	  </div>"
	deviceDetailsHtml += "  </div>";
	
	deviceDetailsHtml += "  <div class='ChinaNet-Form-Sheet ChinaNet-Form-Required'>";
	deviceDetailsHtml += "	<label class='Form-Item-Title ChinaNet-Col-2'>PVLAN</label>";
	deviceDetailsHtml += "	  <div class='Form-Item-Input ChinaNet-Col-9'>";
	deviceDetailsHtml += "		<input type='text' id='input_pvlanid' class='Input-Control'>";
	deviceDetailsHtml += "	  </div>"
	deviceDetailsHtml += "  </div>";
	
	deviceDetailsHtml += "  <div class='ChinaNet-Form-Sheet ChinaNet-Form-Required'>";
	deviceDetailsHtml += "	<label class='Form-Item-Title ChinaNet-Col-2'>CVLAN</label>";
	deviceDetailsHtml += "	  <div class='Form-Item-Input ChinaNet-Col-9'>";
	deviceDetailsHtml += "		<input type='text' id='input_cvlanid' class='Input-Control'>";
	deviceDetailsHtml += "	  </div>"
	deviceDetailsHtml += "  </div>";
	
	deviceDetailsHtml += "  <div class='ChinaNet-Form-Sheet '>";
	deviceDetailsHtml += "	<label class='Form-Item-Title ChinaNet-Col-2'>DOMAIN</label>";
	deviceDetailsHtml += "	  <div class='Form-Item-Input ChinaNet-Col-9'>";
	deviceDetailsHtml += "		<input type='text' id='input_domain' class='Input-Control'>";
	deviceDetailsHtml += "	  </div>"
	deviceDetailsHtml += "  </div>";
	
	deviceDetailsHtml += "  <div class='ChinaNet-Form-Sheet '>";
	deviceDetailsHtml += "	<label class='Form-Item-Title ChinaNet-Col-2'>ETHPORT</label>";
	deviceDetailsHtml += "	  <div class='Form-Item-Input ChinaNet-Col-9'>";
	deviceDetailsHtml += "		<input type='text' id='input_ethport' class='Input-Control'>";
	deviceDetailsHtml += "	  </div>"
	deviceDetailsHtml += "  </div>";
	...
	deviceDetailsHtml += "  <div class='ChinaNet-Form-Sheet'>";
	deviceDetailsHtml += "    <label class='Form-Item-Title ChinaNet-Col-2'>所在地</label>";
	deviceDetailsHtml += "    <div class='Form-Item-Input ChinaNet-Col-9 Form-Item-Disabled'>";
	deviceDetailsHtml += "      <input type='text' id='input_location' class='Input-Control' readonly>";
	deviceDetailsHtml += "    </div>";
	deviceDetailsHtml += "  </div>";
	
	deviceDetailsHtml += "</div>";
	var telecomDetails = dialog({
		id: 'Dailogin:TelecomApDetails',
		title: '编辑电信虚拟设备详细信息',
	...
	telecomDetails.showModal();
	viewDeviceDetails(i);
}

function viewDeviceDetails(i) {
	var deviceId = device_list[i].deviceId;
	var pvlanId = device_list[i].pvlanId;
	var cvlanId = device_list[i].cvlanId;
	var domain = device_list[i].domain;
	var ethPort = device_list[i].ethPort;
	var name = device_list[i].name;
	var telecomAccount = device_list[i].telecomAccount;

	var createDatetime = device_list[i].createDatetime;
	
	var accountId = device_list[i].accoutId;
	var username = device_list[i].username;
	var merchantName = (device_list[i].merchantName == null) ? "" : device_list[i].merchantName;
	var fullname = (device_list[i].fullname == null) ? "" : device_list[i].fullname;
	var accountType = device_list[i].accountType;
	var province = device_list[i].province;
	var city = device_list[i].city;
	var countyDistrict = device_list[i].countyDistrict?device_list[i].countyDistrict:"";
	var address = device_list[i].address;
	var deviceLocation = province + " " + city + " " + countyDistrict;
	$("#input_device_id").val(deviceId);
	$("#input_name").val(name);
	$("#input_telcomaccount").val(telecomAccount);
	$("#input_pvlanid").val(pvlanId);
	$("#input_cvlanid").val(cvlanId);
	$("#input_domain").val(domain);
	$("#input_ethport").val(ethPort);
	$("#input_account_username").val(username);
	$("#input_account_type").val(getAccountTypeCnName(accountType));
	$("#input_create_datetime").val(createDatetime);
	$("#input_location").val(deviceLocation);
	
}

function submitTelecomInfo(i){
	var name = $("#input_name").val();
	var telcomaccount = $("#input_telcomaccount").val();
	var pvlanid = $("#input_pvlanid").val();
	var cvlanid = $("#input_cvlanid").val();
	...
	$.ajax({
		url : '/system/updatetelecomap.htm',
		type : 'POST',
		data : {
		...
		success : function(data) {
			if(data.result == 'OK') {
				...
				deviceSearchHandler.clearResultSetpageNo();
				deviceSearchHandler.searchWithPreload();
				dialog.list['Dailogin:TelecomApDetails'].remove().close();
			} else {
