...
function thirdApplicationListHtml(thirdApplicationList) {
	...
	if (thirdApplicationList.length > 0) {			
		for (var i = 0; i < thirdApplicationList.length; i++) {
			...
			var type = thirdApplicationList[i].typeId;			
			var status = null;
			...
			thirdApplicationListHtml +=   "<td>" ;
			thirdApplicationListHtml +=     "<span class='Table-Data-Name' id='type_" + i + "'>" + _typeData.data[type] + "</span>";
			thirdApplicationListHtml +=   "</td>";
...
function generateNewAppId() {
	$.ajax({
        ...
        success: function (data) {
        	if (data.result == 'OK') {
        		$("#input_new_app_id").val(data.app_id);
        		$("#input_new_app_key").val(data.app_key);
            } else {                   
...
function generateNewAppKey() {
	$.ajax({
        ...
        success: function (data) {
        	if (data.result == 'OK') {
        		$("#input_modify_app_key").val(data.app_key);
            } else {                   
                onAlertError(data.message);
            }
        },
...
function addThirdApplication() {
	...
	thirdApplicationHtml += "    <div id='div_new_app_id' class='Form-Item-Input ChinaNet-Col-7'><input type='text' id='input_new_app_id' class='Input-Control'></div>";
	thirdApplicationHtml += "  </div>";
	
	thirdApplicationHtml += "  <div class='ChinaNet-Form-Sheet'>";
	thirdApplicationHtml += "    <label class='Form-Item-Title ChinaNet-Col-2'>APP KEY</label>";
	thirdApplicationHtml += "    <div id='div_new_app_key' class='Form-Item-Input ChinaNet-Col-7'><input type='text' id='input_new_app_key' class='Input-Control'></div>";
...
function SubmitThirdApplication() {
	...
	var appId = $("#input_new_app_id").val();
	var appKey = $("#input_new_app_key").val();
	var phone = $("#input_new_phone").val();
	var thumbPath = $("#input_new_thumb").val();
	var typeId = $("#input_new_type").val();
	...
	if(!onCheckEmpty(typeId)) {
		onAlertErrorTip('应用分类不能为空！', document.getElementById('input_new_type'));
		return false;
	}
	if(!onCheckEmpty(appId)) {
		onAlertErrorTip('appId不能为空！', document.getElementById('input_new_app_id'));
		return false;
	} else if(appId.length != 8) {
		onAlertErrorTip('appId不足8位！', document.getElementById('input_new_app_id'));
		return false;
	} else if(!onCheckHexadecimal(appId)) {
		onAlertErrorTip('appId格式错误，只能为16进制数！', document.getElementById('input_new_app_id'));
		return false;
	}
	if(!onCheckEmpty(appKey)) {
		onAlertErrorTip('appKey不能为空！', document.getElementById('input_new_app_key'));
		return false;
	} else if(appKey.length != 12) {
		onAlertErrorTip('appKey不足12位！', document.getElementById('input_new_app_key'));
		return false;
	} else if(!onCheckHexadecimal(appKey)) {
		onAlertErrorTip('appKey格式错误，只能为16进制数！', document.getElementById('input_new_app_key'));
		return false;
	}
...
function showThirdApplicationEdit(i) {
	...
	thirdApplicationHtml += "  <div class='ChinaNet-Form-Sheet'>";
	thirdApplicationHtml += "    <label class='Form-Item-Title ChinaNet-Col-2'>APP ID</label>";
	thirdApplicationHtml += "    <div id='div_modify_app_id' class='Form-Item-Input ChinaNet-Col-7'><input type='text' id='input_modify_app_id' class='Input-Control'></div>";
	thirdApplicationHtml += "  </div>";
	
	thirdApplicationHtml += "  <div class='ChinaNet-Form-Sheet'>";
	thirdApplicationHtml += "    <label class='Form-Item-Title ChinaNet-Col-2'>APP KEY</label>";
	thirdApplicationHtml += "    <div id='div_modify_app_key' class='Form-Item-Input ChinaNet-Col-7'><Input type='text' id='input_modify_app_key' class='Input-Control'></div>";
...
function thirdApplicationRefresh(i) {
	...
	$("#input_modify_app_id").val(third_application_list[i].appId);
	$("#input_modify_app_key").val(third_application_list[i].appKey);
	var _initTypeID = third_application_list[i].typeId;
	if(_initTypeID){
...
function ModifyThirdApplication(i) {
	...
	var appId = $("#input_modify_app_id").val();
	var appKey = $("#input_modify_app_key").val();
	...
	if(name == third_application_list[i].name && 
			...
			appId == third_application_list[i].appId &&
	...
	if(!onCheckEmpty(typeId)) {
		onAlertErrorTip('应用分类不能为空！', document.getElementById('input_modify_type'));
		return false;
	}
	if(!onCheckEmpty(appId)) {
		onAlertErrorTip('appId不能为空！', document.getElementById('input_modify_app_id'));
		return false;
	} else if(appId.length != 8) {
		onAlertErrorTip('appId不足8位！', document.getElementById('input_modify_app_id'));
		return false;
	} else if(!onCheckHexadecimal(appId)) {
		onAlertErrorTip('appId格式错误，只能为16进制数！', document.getElementById('input_modify_app_id'));
		return false;
	}
	if(!onCheckEmpty(appKey)) {
		onAlertErrorTip('appKey不能为空！', document.getElementById('input_modify_app_key'));
		return false;
	} else if(appKey.length != 12) {
		onAlertErrorTip('appKey不足12位！', document.getElementById('input_modify_app_key'));
		return false;
	} else if(!onCheckHexadecimal(appKey)) {
		onAlertErrorTip('appKey格式错误，只能为16进制数！', document.getElementById('input_modify_app_key'));
		return false;
	}
	...
	$.ajax({
        ...
        success: function (data) {
        	if (data.result == 'OK') {
            	...
            	dialog.list['Dailogin:ThirdApplicationModify'].remove().close();
            	thirdApplicationSearchHandler.clearResultSetpageNo();
            	thirdApplicationSearchHandler.searchWithPreload();
            } else {                   
