...
function edit_search() {
	var edit_groupId = document.getElementById("edit_groupId").value;
	initUnGroupAp_edit(edit_groupId);	
	checkHasRepeat();
	$("#unHaveRole").bind('dblclick',function(){
		copySelect('unHaveRole','haveRole');
	});
...
<div>
	...
	<div>
		<table class="w50_per">
			...
			<tr>
				<td><input id="edit_groupId" type="text"><button style="margin-left: 10px;" onclick="edit_search()">搜索</button></td>
			</tr>
		</table>
		<ul style="height: 150px;">
			<li id="apUnGroup_li"></li>
	...
	<div>
		<input id="submitButton" name="submit" type="button" onclick="saveGroup('edit')" value="保存"/>
		<input id="backButton" name="back" type="button" value="取消" />
	</div>
</div>
</body>
<!-- 还未被分组的Ap -->
<script id="apUnGroupLi" type="text/x-dot-template">
	<select multiple style="height: 150px; width:330px;"  id="unHaveRole" name="unHaveRole">
	{{~it.records:Ap:index}}
		<option value='{{=Ap.deviceId}}'>{{=Ap.deviceId}}【
			{{? Ap.type == "FIT_AP"}}
				瘦AP
			{{?? Ap.type == "FAT_AP"}}胖AP
			{{?? Ap.type == "BAS"}}专用BAS
...
<!-- 已经被分组的Ap -->
<script id="apGroupLi" type="text/x-dot-template">
