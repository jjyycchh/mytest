...
<div class="wifi-portal-body">
	<div class="portal-menu">
		<a
			...
			class="glyphicon glyphicon-circle-arrow-right"></span></a> <a
			href="${pageContext.request.contextPath}/system/super_searchsms.htm"
			class="active">短信查询<span
			class="glyphicon glyphicon-circle-arrow-right"></span></a> <a
			href="${pageContext.request.contextPath}/user/super_authen_mgmt.htm">用户认证记录<span
			class="glyphicon glyphicon-circle-arrow-right"></span></a> <a
			href="${pageContext.request.contextPath}/system/super_exception_log.htm">异常日志导出
   ...
   //MAIN_MENU.setActiveMenuItem("id_mm_account");
	...
	$(document).ready(function() {
		...
		smsMgsSearchHandler = new searchUtil(generateSMSListHtml, searchFailCallBack, searchErrorCallBack, null, postPageChange,
				...
				"${pageContext.request.contextPath}/system/searchsms.htm", "Msgtab");
	...
	function keywordsSearch() {		
		var keywords = smsMgsSearchHandler.convertKeywordsSearchable($("#keywords").val());
		smsMgsSearchHandler.setSearchParemeter('keywords', keywords);
	...
	function generateSMSListHtml(SMSList) {
		...
		if (SMSList.length > 0) {			
			for (var i = 0; i < SMSList.length;i++) {
				...
				var terminalUserAuthenticationLogId = SMSList[i].terminalUserAuthenticationLogId;
