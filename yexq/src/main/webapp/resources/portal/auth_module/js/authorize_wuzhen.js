...
var AUTHTYPE = 'mobile';
var MESSAGE = {
	SMSwait: ['短信已经发送，请稍等...', 'Access code will arrive in few seconds.'],
	invalidnum: ['请填写正确的手机号码.', 'Cellphone number is invalid.'],
	wait: ['请等待', 'Resend in '],
	second: ['秒', ' sec'],
	getvcode: ['获取验证码', 'Get Access Code'],
	signin: ['点击上网', 'Login'],
	access: ['点击免费上网', 'Click to access free WiFi'],
	authfailed: ['用户验证失败，请重新验证', 'Authorization failed, please try again.'],
	invalidvcode: ['请输入有效的验证码.', 'Access code is invalid.'],
	onlogin: ['正在登录中，请稍后...', 'Authorizing, please wait...']
};
...
$(document).ready(function(){
	...
	$(document).on('click', 'button.AuthMobileSMS', function(){
		if($(this).hasClass('disabled')){
			alert((LANG=='cn'?MESSAGE.SMSwait[0]:MESSAGE.SMSwait[1]));
			return false;
		}

		var mobile = $('input#MobileNumber').val();
		if(mobile.length < 1){
			alert((LANG=='cn'?MESSAGE.invalidnum[0]:MESSAGE.invalidnum[1]));
			return false;
		}
		...
		$.ajax({
			...
			async:false,
			data:{auth_id:mobile,auth_type:'mobile',dev_id:getRequestValue('dev_id'),lang:lang},
			...
			error: function(data){
				...
				//alert('AuthMobileSMS'+str);
			}
		});
	});

	if(typeof(__show_editer)=="undefined"){
		var clientmac = getRequestValue('client_mac') ? getRequestValue('client_mac') : getRequestValue('user_mac');
		if(clientmac){
			...
			$.ajax({
		        ...
		        success:function(data){
					var str = LANG=='cn'?MESSAGE.access[0]:MESSAGE.access[1];
		            if(data.result=='OK'){
                        if($('div.bottombutton').length>0){
                            $('div.bottombutton').css({'padding':0}).html('<button type="button" class="AuthSubmitID ValidateInMobile optionbutton">'+str+'</button>');
                        }else{
                            if($('div.Portal-Auth-Body').length>0) $('div.Portal-Auth-Body').html('<div class="Auth-Type-Free"><button type="button" class="AuthSubmitID ValidateInMobile">'+str+'</button></div>');
							//inMobileAutoLogin();
                        }
		            }
		        },
...
function resetSendSMS(){
	if(SMSSECO>0){
		SMSSECO = SMSSECO-1;
		$('button.AuthMobileSMS').html((LANG=='cn'?MESSAGE.wait[0]:MESSAGE.wait[1])+SMSSECO+(LANG=='cn'?MESSAGE.second[0]:MESSAGE.second[1]));
		SMSTIME = window.setTimeout('resetSendSMS()', 1000);
	}else{
		window.clearTimeout(SMSTIME);
		SMSSECO = 120;
		$('button.AuthMobileSMS').removeClass('disabled');
		$('button.AuthMobileSMS').html((LANG=='cn'?MESSAGE.getvcode[0]:MESSAGE.getvcode[1]));
	}
}

function resetLogin(obj, time){
	if(typeof time != 'undefined'){
		LOGINSEC = time;
	}
	var title = $(obj).html();
	var secpos = title.indexOf('(');
	if(secpos > -1){
		title = title.substr(0, secpos);
	}
	if(LOGINSEC == 0){
		$(obj).html(title);
		$(obj).attr('disabled', false);
	}else{
		$(obj).html(title+'('+LOGINSEC+')');
...
function inMobileAutoLogin(){
	var $btn = $('button.AuthSubmitID');
	$btn.attr('disabled', true);
	resetInMobile($btn.get(0), 5);
	onValidatorInMobile();
}

function resetInMobile(obj, time){
	if(typeof time != 'undefined'){
		LOGINSEC = time;
	}
	var title = LANG=='cn' ? MESSAGE.onlogin[0]:MESSAGE.onlogin[1];
	var btntitle = LANG=='cn' ? MESSAGE.access[0]:MESSAGE.access[1];
	if(LOGINSEC == 0){
		$(obj).html(btntitle);
		$(obj).attr('disabled', false);
	}else{
    ...
    if(type.length>1){
        ...
        if(type.length==1) {
            if (type.attr('id').toLowerCase() == 'option') {
                $('div.auth-component-button').show().find('button').attr('class', 'fee-wifi-config').html((LANG=='cn'?MESSAGE.access[0]:MESSAGE.access[1]));
...
function authRequest(param_data, destUrl, onSuccessLocation){
	if (((window.location)+'').indexOf('api10')>-1) {
		...
		$.ajax({
			...
			success:function(data){
				if(data.result=='OK'){
					var lang = LANG=='en' ? '&lang=en' : '';
					if(!getRequestValue('gw_address')||getRequestValue('gw_address')==''){
						window.location = 'http://'+location.hostname+':'+location.port+'/api10/portal?'+location.search.substr(1)+lang;
					}else{
						...
						window.location = onSuccessLocation + data.token + lang;
					}
				}else{
					alert((LANG=='cn'?MESSAGE.authfailed[0]:MESSAGE.authfailed[1]));
				}
			},
			error: function(data){
				//var str = JSON.stringify(data);
		...
		$.ajax({
			...
			success:function(data){
				if(data.result=='OK'){
					var lang = LANG=='en' ? '&lang=en' : '';
					if(!getRequestValue('gw_address')||getRequestValue('gw_address')==''){
						window.location = 'http://'+location.hostname+':'+location.port+'/site/portal/?'+location.search.substr(1)+lang;
					}else{
...
function aciAuthRequest(mobile, captcha, auth_type){
	...
	$.ajax({
		...
		success:function(data){
			if(data.result=='OK'){
				var lang = LANG =='en' ? '&lang=en' : '';
				window.location = 'http://'+location.hostname+':'+location.port+ "/aci10/portal?dev_id=" + data.dev_id + lang;
			}else{
				LOGINSEC = 1;
				alert((LANG=='cn'?MESSAGE.authfailed[0]:MESSAGE.authfailed[1]));
			}
...
function onValidatorMobile(){
	...
	if(captcha==''){
		alert((LANG=='cn'?MESSAGE.invalidvcode[0]:MESSAGE.invalidvcode[1]));
		return false;
	}
