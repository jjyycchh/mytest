...
var LANG = 'cn';
var LANG_DATA = [];
var appLink = ['http://iwifi.9conn.net/app.htm', 'http://iwifi.9conn.net/app_ios.htm'];
...
var MyTemplate = function(){
	...
	var initIcons = function(){
		var iconHeight = 153*MAX_SCALE;
		$('#siteImageLinks .siteComponent').height(iconHeight);
		if(isIOS())
			$('#imageLink05 a').attr('href', appLink[1]);
		else
			$('#imageLink05 a').attr('href', appLink[0]);
	};
	...
	var chgLanguage = function(obj){
		...
		if(lang == 'cn'){
			...
			$('#loginStatus').removeClass('Login-En');
			for(var i=0; i<LANG_DATA.length; i++){
				if(LANG_DATA[i][2] == 'placeholder'){
