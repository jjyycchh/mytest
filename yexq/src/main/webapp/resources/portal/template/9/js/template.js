...
var MyTemplate = function(){
	...
	var loadImageText = function(){
		...
		var width   = _this.find('div#siteImageTextModuleBox').width()/2;
	...
	var playOverlyer = function(){
		$('div#siteSliderModuleBox').smallslider({width:PTPL_MAX_WIDTH,height:PTPL_MAX_WIDTH/3*2,scale:MAX_SCALE});
		if(!LOAD_OVERLY&&!CUT_THUMB){	
			//alert(_this.height());
			$('#portalTemplateBody .siteModules').hide();
...
$(document).ready(function(){
	...
	$(document).on('click', 'a#compeditdom', function(){
		if(!$(this).hasClass('disabled')){
			...
			if(__edit_moduleid=='siteLogoViewModuleBox'||__edit_moduleid=='siteSliderModuleBox'||__edit_moduleid=='siteImageSpanModule'){
				var loadhtml = $(editImageTextModuleHtml);
			}else if(__edit_moduleid=='siteTextBlockModuleBox'||__edit_moduleid=='siteArticleViewModuleBox'||__edit_moduleid=='siteFooterGGModuleBox'||__edit_moduleid=='siteFooterModuleBox'||__edit_moduleid=='siteColumnModuleBox2'||__edit_moduleid=='siteColumnModuleBox1'){
