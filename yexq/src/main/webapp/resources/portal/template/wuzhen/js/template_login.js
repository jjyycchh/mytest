...
var MyTemplate = function(){
	...
	var LocationClk = function(){
		if(!getRequestValue('url')||getRequestValue('url')==''){
			...
			$("div#siteLocationModuleBox").hide();
		}
		else{
			$("div#siteLocationModuleBox").show();
	...
	var loadImageText = function(){
		...
		var width   = _this.find('div#siteImageTextModuleBox').width();
			_this.find('div#siteImageTextModuleBox img').css({width:95,height:95});
	...
	var initLanguage = function(){
		//LANG_DATA.push(['title', {cn:$('title').text(), en:'Wuzhen'}, 'text']);
		LANG_DATA.push(['a.android', {cn:$('a.android').text(), en:'Android'}, 'text']);
		LANG_DATA.push(['a.ios', {cn:$('a.ios').text(), en:'iPhone'}, 'text']);
		LANG_DATA.push(['#siteColumnModuleBox2 span', {cn:$('#siteColumnModuleBox2 span').text(), en:'iWifi client download'}, 'text']);
		LANG_DATA.push(['#loginStatus span', {cn:$('#loginStatus span').text(), en:'Login successfully<br/>You can access the internet now!'}, 'text']);
		LANG_DATA.push(['#imagetext01 #title', {cn:$('#imagetext01 #title').text(), en:'World Internet Conference'}, 'text']);
		LANG_DATA.push(['#imagetext01 #description', {cn:$('#imagetext01 #description').text(), en:'Centering on the theme of "An Interconnected World Shared and Governed by All", leading figures all around the world from governments, international organizations, enterprises, science and technology communities as well as civil societies will discuss and exchange ideas deeply. Together let us open the new historical chapters in which the development of the Internet contributes to equitable development and improvement of human welfare.'}, 'text']);
		LANG_DATA.push(['#imagetext02 #title', {cn:$('#imagetext02 #title').text(), en:'About Wuzhen'}, 'text']);
		LANG_DATA.push(['#imagetext02 #description', {cn:$('#imagetext02 #description').text(), en:'Wuzhen, a 1300-year-old water town on the lower reaches of the Yangtze River, is a national 5A scenic area and one of China’s top ten historical & cultural towns. Located on the Hangzhou-Jiaxing-Huzhou Plain in northern Zhejiang Province, it is at the center of the golden triangle consisting of Shanghai, Hangzhou and Suzhou.'}, 'text']);
		LANG_DATA.push(['#imagetext03 #title', {cn:$('#imagetext03 #title').text(), en:'Tongxiang Telcom Wechat'}, 'text']);
		LANG_DATA.push(['#imagetext03 #description', {cn:$('#imagetext03 #description').text(), en:'Follow Tongxiang Telcom on Wechat for more promotions by scanning QR code or searching: TXDX3G'}, 'text']);
	};
	var chgLanguage = function(obj){
		var lang = $(obj).attr('lang');
		if(lang == 'cn'){
			LANG = 'en';
			$(obj).attr('lang', 'en').html('中文');
			$('#loginStatus').addClass('Login2-En');
			for(var i=0; i<LANG_DATA.length; i++){
				if(LANG_DATA[i][2] == 'placeholder'){
					$(LANG_DATA[i][0]).attr('placeholder', LANG_DATA[i][1].en);
				}else{
					$(LANG_DATA[i][0]).html(LANG_DATA[i][1].en);
				}
			}
		}else{
			LANG = 'cn';
			$(obj).attr('lang', 'cn').html('English');
			$('#loginStatus').removeClass('Login2-En');
			for(var i=0; i<LANG_DATA.length; i++){
				if(LANG_DATA[i][2] == 'placeholder'){
					$(LANG_DATA[i][0]).attr('placeholder', LANG_DATA[i][1].cn);
				}else{
					$(LANG_DATA[i][0]).html(LANG_DATA[i][1].cn);
				}
			}
		}
	};
	return {init:function(editmode){
		...
		if(document.getElementById('lang')){
			...
			if(Request['lang'] == 'en') chgLanguage(document.getElementById('lang'));
		}
		getMaxWidth();
		loadImageText();		
		//imageSpanBlock();
