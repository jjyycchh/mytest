...
var MyTemplate = function(){
	...
	var generateUserInfo = function(){
		if (!LOAD_OVERLY && !CUT_THUMB) {
			...
			var macstr = isNotEmptyString(getRequestValue('client_mac'))?getRequestValue('client_mac').replace(/:/g,"").toUpperCase():'';
			//$("#zhengchang").removeClass("zhengchangcss").addClass('zhengchangcss_o');
				$.ajax({
					url:authurl+'/user/logininfo.htm',
					type:'GET',
					dataType:'JSONP',
					jsonp:"getResultRequest",
					async:false,
					data:{device_id:getRequestValue('dev_id'),mac:macstr},
					success:function(data){
						if(data.result=='OK'){											
							$("#lb_onlines").text(data.loginCount);
							$("#lb_linknum").text(data.loginTimes);
							
							if(data.netState>0.7){
								$("#baoman").removeClass("baomancss").addClass('baomancss_o');
							}else if(data.netState<0.2){
								$("#kongxian").removeClass("kongxiancss").addClass('kongxiancss_o');
							}else{
								$("#zhengchang").removeClass("zhengchangcss").addClass('zhengchangcss_o');
							}
						}
	...
	var initLanguage = function(){
		...
		LANG_DATA.push(['#MobileNumber', {cn:$('#MobileNumber').attr('placeholder'), en:'Cellphone number'}, 'placeholder']);
		LANG_DATA.push(['#SMSCodeNumber', {cn:$('#SMSCodeNumber').attr('placeholder'), en:'Access code'}, 'placeholder']);
		LANG_DATA.push(['button.AuthMobileSMS', {cn:$('button.AuthMobileSMS').text(), en:'Get access code'}, 'text']);
		LANG_DATA.push(['button.ValidatorMobile', {cn:$('button.ValidatorMobile').text(), en:'Login'}, 'text']);
		LANG_DATA.push(['button.ValidatorOption', {cn:'点击免费上网', en:'Click to access free WiFi'}, 'text']);
		LANG_DATA.push(['button.ValidateInMobile', {cn:'点击免费上网', en:'Click to access free WiFi'}, 'text']);
		LANG_DATA.push(['a.android', {cn:$('a.android').text(), en:'Android'}, 'text']);
		LANG_DATA.push(['a.ios', {cn:$('a.ios').text(), en:'iPhone'}, 'text']);
		LANG_DATA.push(['#siteColumnModuleBox1 span', {cn:$('#siteColumnModuleBox1 span').text(), en:'iWifi - Free WiFi'}, 'text']);
	...
	var chgLanguage = function(obj){
		...
		if(lang == 'cn'){
			...
			$('.Portal-Auth-Body').addClass('Login-En');
			...
			$('.Portal-Auth-Body').removeClass('Login-En');
			for(var i=0; i<LANG_DATA.length; i++){
				if(LANG_DATA[i][2] == 'placeholder'){
