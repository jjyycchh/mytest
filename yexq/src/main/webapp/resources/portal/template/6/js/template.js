...
var MyTemplate = function(){
	...
	var playOverlyer = function(){
		...
		//	var image = _this.find('div#siteLoadingModuleBox img')[0];
			...
			if (image.width > 100 && image.height > 100) {
				...
				if (image.width / image.height >= _width / _height) {
					if (image.width > _width) {
						l_width = _width;
						l_height = (image.height * _width) / image.width;
					} else {
						l_width = image.width;
					...
					if (image.height > _height) {
						l_height = _height;
						l_width = (image.width * _height) / image.height;
					} else {
						l_width = image.width;
						l_height = image.height;
					}
...
$(document).ready(function(){
	...
	$(document).on('click', 'a#compeditdom', function(){
		if(!$(this).hasClass('disabled')){
			...
			}else if(__edit_moduleid=='siteTextBlockModuleBox'||__edit_moduleid=='siteArticleViewModuleBox'){
