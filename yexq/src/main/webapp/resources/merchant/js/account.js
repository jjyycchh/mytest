...
var accountApp = function(){
            ...
            */          
            ...
            $.ajax({
                ...
                success:function(data){
                    if(data.result=='OK'){
                        $.ajax({
                            url:'/account/viewaccountdetails26.htm?accountId='+data.accountId,
                            type:'GET',
                            dataType:'HTML',
                            async:false,
    ...
    var onCancelAccountEdit = function(){
        $('button#AccountEditCancel').click(function(){
            if(_AccountData!=null){
                $.ajax({
                    url:'/account/viewaccountdetails26.htm?accountId='+_AccountData.id,
                    type:'GET',
                    dataType:'HTML',
                    async:false,
                    success:function(data){
                ...
                $.ajax({
                    url:'/account/account_management26.htm',
                    type:'GET',
                    dataType:'HTML',
                    async:false,
                    success:function(data){
...
function generateAcctListHtml(accountList) {
	...
	if (accountList.length > 0) {
		...
		for (var i = 0; i < accountList.length;i++) {
			...
			acctListHtml +=     "<td><a href='viewaccountdetails26.htm?accountId="+id+"' data-public-params='"+id+"' class='initAjax Table-Data-Name-Link'>" + username + "</a>";
