...
var portalApp = function(){
            ...
            $.ajax({
                ...
                success:function(data){
                    if(data.result=="OK"){
                        ...
                        $('#SiteStrategyNubmer').html(data.sitedetails.refPolicyNum);
    ...
    var changeEditData = function(){
                ...
                if(!isNaN(pageid)) {
                    if(onSetPageDataValue(pageid)){
                        ...
                        if(html.length>0){
                            ...
                            $('iframe#PageEditorIframeID').attr('src', '/merchant/load_pagedata26.htm?sid='+Math.random());
                            dom.write(html);
                            dom.close();
        ...
        $('a#SaveEditorOnPage').click(function(){
            if($('iframe#PageEditorIframeID').attr('src')=='/merchant/templates26.htm'){
            ...
            if(typeof _EditPageData!='undefined'&&_EditPageData!=null){
                ...
                if(typeof _SiteData!='undefined'&&_SiteData!=null){
                    if(onCheckPageType({pageid:_EditPageData.pageid,type:$('input#PageTemplateType').val()})){
                        ...
                        if(result.result=="OK"){
                            $.ajax({
                                url:'/merchant/sitehaspolicy.htm',
                                type:'GET',
                                async:false,
                                data:{
                                ...
                                success:function(data){
                                    if(data.result=='FAIL' && data.devices.length !=0){ // 尚未绑定策略
                                        closeLoading();
                                        var bindPolicyHtml;
                                        if(data.devices.length == 1){
                                            bindPolicyHtml = [
                                                '<div class="Bind-Policy-Body">您只有一个wifi设备，是否为其创建该站点的默认推送策略？</div>'
                                            ];
                                            var bindPolicyShow = dialog({
                                                id: 'Dailogin:bindPolicyShow',
                                                __iwifi_dev: data.devices[0],
                                                title: '创建站点默认推送策略',
                                                content: bindPolicyHtml.join(''),
                                                okValue: '是',
                                                ok: function(){
                                                    return siteBindPolicy([this.__iwifi_dev.device_id]);
                                                },
                                                cancelValue: '否',
                                                cancel: function(){
                                                    onSiteSaveComplete();
                                            ...
                                            bindPolicyShow.showModal();

                                        }else{
                                            bindPolicyHtml = [
                                                '<div class="Bind-Policy-Body">',
                                                    '<div class="ChinaNet-Form-Sheet">',
                                                        '<label class="Form-Item-Title ChinaNet-Col-2">策略设备：</label>',
                                                        '<div class="ChinaNet-Left ChinaNet-Col-10">',
                                            ...
                                            var bindPolicyShow = dialog({
                                                ...
                                                ok: function(){
                                                    ...
                                                    if(deviceids.length == 0){
                                                        onAlertErrorTip('请至少选择一个设备！', $('div.Portal-Policy-Devices-List')[0]);
                                                        return false;
                                                    }
                                                    return siteBindPolicy(deviceids);
                                                },
                                                cancelValue: '取消',
                                                cancel: function(){
                                                    onSiteSaveComplete();
                                ...
                                error: function(status){
                                    console.log('/merchant/sitehaspolicy26.htm...error!');
                                    onSiteSaveComplete();
                                }
                            });
    ...
    var onPolicyDeviceArea = function(){
        $('div#portalPolicyDeviceArea a').off('click').on('click', function(){
            if($(this).hasClass('Add-Device')){
                var content = '' +
        ...
        // 删除设备
        $('div#portalPolicyDeviceArea a span').die('click').live('click', function(){
            var device = $(this);
        ...
        // 选中/取消选中设备
        $('div.Policy-Device-Sheet-Body ul li').die('click').live('click', function(){
            ...
            if($(this).hasClass('Active')){
                ...
                $('div.Devices-List-Item-Body a.Add-Device').before('<a href="javascript:;" data-deviceId="'+id+'"><span>&nbsp;</span>' + $(this).attr('data-deviceName') + '</a>');
            }
        });
     ...
     * 保存推送策略
     */
    var siteBindPolicy = function(devs){
        var policy = {};
        var flag   = false;
        policy.policyid   = '';
        policy.policyname = _SiteData.sitename + '默认策略';
        policy.accountid  = __CONTEXT_MERCHANT_KEY;
        policy.policyitems = JSON.stringify([
            {
                "siteid": _SiteData.siteid,
                "sitename": _SiteData.sitename,
                "starttime": "0",
                "endtime": "23",
                "priority":1
            }
        ]);
        policy.deviceids  = JSON.stringify(devs);
     ...
     * 保存策略成功
     */
    var onPolicyComplete = function(){
        bindPolicySucHtml = [
            '<div class="Bind-Policy-Body">站点：<span class="Bind-Policy-Highlight">', _SiteData.sitename + '默认策略', '</span>，已经成功创建默认推送策略。</div>'
        ];
        var bindPolicySucShow = dialog({
            id: 'Dailogin:bindPolicySucShow',
            title: '创建站点默认推送策略',
            content: bindPolicySucHtml.join(''),
            okValue: '关闭',
            ok: function(){
                onSiteSaveComplete();
            },
        ...
        bindPolicySucShow.showModal();
    }
    /**
     * 保存站点结束
     */
    var onSiteSaveComplete = function(){
        $.ajax({
            url:'/merchant/editsite26.htm?siteid=' + _SiteID,
            type:'GET',
            dataType:'HTML',
            async:false,
            success:function(data){
    ...
    var onChangeTemplateType = function(){
        $('div#PortalTemplateTypeMenu a').click(function(){
            ...
            $('#PageEditorIframeID').attr('src', '/merchant/templates26.htm');
    ...
    var onSubmitSiteData = function(){
        $('form#FormSiteDetialSave').submit(function(){
            ...
            if(result.result=="OK"){
                $.ajax({
                    url:'/merchant/editsite26.htm?siteid='+result.siteid,
                    type:'GET',
                    dataType:'HTML',
                    async:false,
                    success:function(data){
...
function onSetPageDataValue(pageid){
    if(onSetEditPortalPage(pageid)) {
        ...
        $('iframe#PageEditorIframeID').attr('src', '/merchant/load_pagedata26.htm');
...
function onAddNewPageForSite(){
    ...
    $('iframe#PageEditorIframeID').attr('src', '/merchant/templates26.htm');
    $('div.ChinaNet-Portal-Editor-Page').slideDown(100);
    $('div.ChinaNet-Portal-Detail-Body').stop().animate({height:80},100);
    return true;
}
	...
	if (siteList.length > 0) {
		for ( var i = 0; i < siteList.length; i++) {
			...
			var thumburl = isNotEmptyString(siteList[i].thumb_url)?siteList[i].thumb_url : "/statics/img/no-image.png";
                ...
                //siteListHtml += "    		<a href='/merchant/editsite26.htm?siteid="+siteid+"' class='initAjax'>编辑</a>";
...
function generateSiteOptBtn(siteId,Type)
{
	...
	if (siteId != null) {		
		if (Type == "LOCKED") {
			...
			siteOptBtnHtml += "<a href='/merchant/editsite26.htm?siteid="+siteId+"' class='initAjax'>编辑</a>";
		}
